    ...
    <SectionWrapper id="metricas">
      <div className="mb-16 text-center">
        ...
        <h2 className="mb-5 font-heading text-3xl font-bold text-slate-900 sm:text-4xl lg:text-4.5xl">
          Los números que{' '}
          <span className="text-emerald-500">realmente importan</span>
        </h2>
      ...
      <div className="mb-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {metrics.map((metric) => (
          <div
            key={metric.label}
            className="group rounded-[1.5rem] border border-slate-200/60 bg-white p-7 text-center shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-card-3d"
          >
      ...
      <div className="rounded-[2rem] border border-slate-200/60 bg-white p-6 shadow-card sm:p-8">
        <div className="mb-6 flex items-center gap-2.5">
          ...
          <h3 className="text-lg font-bold text-slate-900">Semáforo de salud</h3>
        </div>
          ...
          {healthIndicators.map((indicator) => {
            ...
            return (
              <div key={indicator.label} className={`rounded-2xl ${style.bg} p-5`}>
                ...
                <p className="text-xs text-slate-400">{indicator.threshold}</p>
              </div>
            )
          })}
        </div>
